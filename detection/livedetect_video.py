...
try:
    import PIL.Image
    # Monkey patch ANTIALIAS if it doesn't exist
    if not hasattr(PIL.Image, 'ANTIALIAS'):
        PIL.Image.ANTIALIAS = PIL.Image.Resampling.LANCZOS
    logging.info("PIL compatibility patch applied successfully")
except Exception as e:
    logging.warning(f"PIL compatibility patch failed: {e}")
...
# EasyOCR setup with CPU by default (GPU causes issues on some systems)
try:
    reader = easyocr.Reader(['en'], gpu=False)  # Use CPU by default
    logging.info("EasyOCR initialized successfully with CPU")
except Exception as e:
...
def save_slot_status_to_file():
    ...
    try:
        ...
        logging.info("slot_status.json written: " + json.dumps(serializable_status))
    except Exception as e:
            ...
            for j, config in enumerate(ocr_configs):
                ...
                if ocr_results:
                    ...
                    if extracted_text:
                        ...
                        avg_confidence = sum(res[2] for res in ocr_results) / len(ocr_results)
    ...
    try:
        ...
        for i, (bbox, text, conf) in enumerate(ocr_results):
            if conf >= MIN_CONFIDENCE:
                ...
                scaled_bbox = []
                for point in bbox:
                    scaled_x = int(point[0] * display_width / w)
                    scaled_y = int(point[1] * display_height / h)
        ...
        while cap.isOpened() and is_detection_running:
            ...
            for slot_id in PARKING_SLOTS:
                ...
                if current_occupancy[slot_id]:
                    # Vehicle is currently detected - update last seen time
                    buffer["last_seen"] = current_time
                    buffer["buffer_start"] = None
                elif status["occupied"] and buffer["last_seen"]:
                    # Vehicle was previously detected but not currently seen
                    if buffer["buffer_start"] is None:
                        ...
                        # Buffer time elapsed - mark slot as free
                        plate_info = f" (Plate: {status['license_plate']})" if status['license_plate'] else " (No plate detected)"
                ...
                if current_occupancy[slot_id] and not status["occupied"]:
                    ...
                    # Reset buffer for this slot
                    buffer["last_seen"] = current_time
                    buffer["buffer_start"] = None
                    save_slot_status_to_file()

                # Handle OCR processing for occupied slots
            ...
            for slot_id, area in PARKING_SLOTS.items():
                ...
                # Determine color based on status and buffer
                if not status["occupied"]:
                    color = (0, 255, 0)  # Green for free
                elif buffer["buffer_start"] is not None:
                    color = (0, 165, 255)  # Orange for buffer (vehicle leaving)
                else:
                ...
                if status["occupied"]:
                    ...
                    if buffer["buffer_start"] is not None:
                        # Show buffer countdown
                        buffer_remaining = max(0, buffer["buffer_duration"] - (current_time - buffer["buffer_start"]))
                        text += f"{vehicle_type} - Leaving... ({buffer_remaining:.1f}s)"
                    elif status["license_plate"]:
