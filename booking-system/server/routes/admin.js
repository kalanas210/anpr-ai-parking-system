...
router.get('/bookings', async (req, res) => {
  try {
    const { page = 1, limit = 20, status, date, startDate, endDate, slotNumber, customerName, paymentStatus } = req.query;
    ...
    if (customerName) {
      query['customerDetails.name'] = { $regex: customerName, $options: 'i' };
    }
...
router.get('/statistics', async (req, res) => {
  try {
    ...
    // Slot occupancy for today
    const slotOccupancy = await Booking.aggregate([
      {
        $match: {
          date: today,
          status: { $in: ['confirmed', 'completed'] }
...
router.put('/bookings/:id/status', async (req, res) => {
  try {
    ...
    // Update actual times if completing
    if (status === 'completed' && !booking.actualArrivalTime) {
    ...
    try {
      if (status === 'completed' && previousStatus !== 'completed') {
        await smsService.sendBookingCompletion(booking);
        console.log(`SMS completion sent for booking ${booking.orderId}`);
...
// @desc    Get slot conflicts (for monitoring)
// @route   GET /api/admin/slot-conflicts
// @access  Private/Admin
router.get('/slot-conflicts', async (req, res) => {
  try {
    ...
    const conflicts = await Booking.aggregate([
      ...
      {
        $match: {
          'bookings.1': { $exists: true } // More than one booking
        }
      }
    ]);
    ...
    console.error('Get slot conflicts error:', error);
    res.status(500).json({
      success: false,
      message: 'Server error'
    });
